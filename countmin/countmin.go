...
func (sketch *Sketch) makeHashes() []hash.Hash32 {
	hashes := make([]hash.Hash32, len(sketch.rows))
	for i := range hashes {
		hashes[i] = fnv.New32()
		// Seed with four bytes from π.
		binary.Write(hashes[i], binary.BigEndian, π[i])
	}
	return hashes
}

// Add one observation of key to the sketch.
func (sketch *Sketch) Add(key []byte) {
...
func (sketch *Sketch) AddNGram(key []string) {
	hashes := sketch.makeHashes()
...
// Point query for observations of key. Returns an approximate count.
func (sketch *Sketch) Get(key []byte) (count uint32) {
	hashes := sketch.makeHashes()
